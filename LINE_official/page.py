...
def updateInfo():
    ...
    for idx, user in enumerate(usrInfo):
        ...
        # Right column: Name
        name_label = tk.Label(F_userList, text=user[2], bg=None, font=("Arial", 14))
...
def init_main_page():
    ...
    B_renew = tk.Button(F_top, text="renew", command=renew, image=P_renew)
    ...
    B_setting = tk.Button(F_top, text="setting", command=open_settings, image=P_setting)
...
def init_settings_page():
    ...
    B_confirm = tk.Button(bottom_frame, text="確認", command=save_settings)
